...
const stack = [
  { icon: '📱', title: 'Cliente — Nuxt 4 (PWA / Capacitor)', text: 'Captura, Panel 360, Red viva y Métricas; instalable como app y con modo offline.' },
  { icon: '⚙️', title: 'Backend — FastAPI', text: 'Agente GraphRAG con tool calling, autenticación JWT + refresh rotativo (RBAC) y canal WS de eventos.' },
  { icon: '🧠', title: 'Inferencia — QVAC (Tether)', text: 'MedPsy Q4_K_M (~4.5 GB) + EmbeddingGemma multilingüe sobre RTX; servidor OpenAI-compatible, cero nube.' },
  { icon: '🗂️', title: 'Datos — Neo4j + PostgreSQL', text: 'Grafo de la red instalada y estado operativo con log inmutable de transacciones.' },
  { icon: '🎙️', title: 'Voz — speaches / faster-whisper', text: 'Transcripción en CPU a propósito: la GPU se reserva para el modelo de chat.' },
  { icon: '🛡️', title: 'Gateway — Caddy único punto público', text: 'Un solo origen para app, /api/* y /ws/*; Neo4j, Postgres y QVAC son internos.' },
]

const value = [
  { icon: '📋', title: 'Inventario vivo sin formularios', text: 'El parque instalado se actualiza hablando, no rellenando campos: la fricción de la captura desaparece.' },
  { icon: '✅', title: 'Confianza por consenso', text: 'Cada dato gana fiabilidad a medida que distintos observadores lo confirman; nada se presenta como hecho sin respaldo.' },
  { icon: '🔁', title: 'Oportunidades de renovación', text: 'Antigüedad, modalidades y estado por instalación hacen visible dónde hay ciclo de renovación tecnológica.' },
  { icon: '🔒', title: 'Soberanía de los datos', text: 'Hospital e instalaciones conservan sus datos en su propio hardware; nada sale a APIs de terceros.' },
  { icon: '💸', title: 'Coste de despliegue mínimo', text: 'Una RTX 3060 Ti (8 GB) aloja todo el stack; corre igual en CPU para desarrollo.' },
  { icon: '🌐', title: 'Red de pares', text: 'Los nodos pueden sincronizar observaciones entre sí (SYNC_PEERS): cada sede ve la red completa sin centralizarla.' },
]
...
<template>
  <div class="flex flex-col gap-4">
    <div>
      <h1 class="text-2xl font-bold text-white">Documentación</h1>
    ...
    <div class="glass p-4 sm:p-5">
      <h2 class="text-sm font-semibold text-white">¿Qué es este proyecto?</h2>
      <p class="mt-2 text-xs leading-relaxed text-slate-300">
        ...
        e instalaciones— en una base de datos <strong class="text-white">GraphRAG</strong> viva, estructurada y
        confiable. Toda la inferencia corre <strong class="text-white">on-edge</strong> con QVAC sobre hardware
    ...
    <!-- Cómo funciona -->
    <div class="glass p-4 sm:p-5">
      <h2 class="text-sm font-semibold text-white">¿Cómo funciona?</h2>
      <ol class="mt-3 flex flex-col gap-3">
        <li v-for="(step, i) in howItWorks" :key="step.title" class="flex items-start gap-3">
          <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-indigo-400/20 text-xs font-bold text-indigo-200">
            {{ i + 1 }}
          </span>
          <div class="min-w-0">
            <p class="text-xs font-semibold text-white">{{ step.title }}</p>
            <p class="mt-0.5 text-xs leading-relaxed text-slate-400">{{ step.text }}</p>
          </div>
        </li>
      </ol>
    </div>
    ...
    <!-- En qué se sostiene -->
    <div class="glass p-4 sm:p-5">
      <h2 class="text-sm font-semibold text-white">En qué se sostiene</h2>
      <p class="mt-1 text-xs text-slate-400">
        Stack 100 % local: un único gateway público y todos los servicios internos en la misma red.
      </p>
      <div class="mt-3 grid gap-2 sm:grid-cols-2">
        <div v-for="s in stack" :key="s.title" class="flex items-start gap-2.5 rounded-xl border border-white/10 bg-white/5 px-3 py-2.5">
          <span class="text-lg" aria-hidden="true">{{ s.icon }}</span>
          <div class="min-w-0">
            <p class="text-xs font-semibold text-white">{{ s.title }}</p>
            <p class="mt-0.5 text-[11px] leading-relaxed text-slate-400">{{ s.text }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- Valor del producto -->
    <div class="glass p-4 sm:p-5">
      <h2 class="text-sm font-semibold text-white">Valor del producto</h2>
      <ul class="mt-3 grid gap-2 md:grid-cols-2">
        <li v-for="v in value" :key="v.title" class="rounded-xl border border-white/10 bg-white/5 px-3 py-2.5">
          <p class="text-xs font-semibold text-white">{{ v.icon }} {{ v.title }}</p>
          <p class="mt-0.5 text-[11px] leading-relaxed text-slate-400">{{ v.text }}</p>
        </li>
      </ul>
    </div>
    ...
    <div class="glass p-4 sm:p-5">
      <div class="flex flex-wrap items-center justify-between gap-2">
        <h2 class="text-sm font-semibold text-white">Follow-ups · Seguimientos propuestos</h2>
      ...
      <ul class="mt-4 grid gap-3 md:grid-cols-2">
        <li v-for="fu in followUps" :key="fu.title" class="rounded-2xl border border-white/10 bg-white/5 p-4">
          <div class="flex items-start gap-3">
            <span class="text-2xl" aria-hidden="true">{{ fu.icon }}</span>
            <div class="min-w-0">
              <p class="text-sm font-semibold text-white">{{ fu.title }}</p>
              <p class="mt-1 text-xs leading-relaxed text-slate-300">{{ fu.summary }}</p>
            </div>
          </div>
          <ul class="mt-3 flex flex-col gap-1.5">
            <li v-for="d in fu.details" :key="d" class="flex items-start gap-1.5 text-xs text-slate-400">
              <span class="mt-1 h-1 w-1 shrink-0 rounded-full bg-indigo-400" />
              {{ d }}
            </li>
          </ul>
          <div class="mt-3 flex flex-wrap gap-1.5">
